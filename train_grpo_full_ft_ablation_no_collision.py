"""
Ablation: shape + interlocking + connectivity optimize training, while collision reward is logged only.
...
OUTPUT_DIR = os.getenv("OUTPUT_DIR", "finetuned_models/qwen_pc_grpo_full_ft_ablation_no_collision")
RUN_NAME = os.getenv("RUN_NAME", "brickgpt-grpo-full-ft-ablation-no-collision")
...
def main():
    ...
    grpo_config = GRPOConfig(
        ...
        # Keep logging collision_reward, but exclude it from the optimization objective.
        reward_weights=[0.0, 1.0, 1.0, 1.0],
    )
    ...
    if resume_checkpoint is not None:
        print(f"Resuming GRPO full FT (no-collision ablation) from {resume_checkpoint}")
        trainer.train(resume_from_checkpoint=resume_checkpoint)
    else:
        trainer.train()
    trainer.save_model(OUTPUT_DIR)
    print(f"GRPO full FT (no-collision ablation) saved to {OUTPUT_DIR}")
