"""
Base-model full fine-tuning GRPO training script for BrickGPT.
Starts directly from the base Qwen model and updates full model weights (no SFT adapter, no LoRA).
"""
import os
import sys
...
OUTPUT_DIR = os.getenv("OUTPUT_DIR", "./finetuned_models/qwen_pc_grpo_base_full_ft")
RUN_NAME = os.getenv("RUN_NAME", "brickgpt-grpo-base-full-ft")
...
def main():
    ...
    if resume_checkpoint is not None:
        print(f"Resuming base-model GRPO full FT from {resume_checkpoint}")
        trainer.train(resume_from_checkpoint=resume_checkpoint)
    else:
        trainer.train()
    trainer.save_model(OUTPUT_DIR)
    print(f"Base-model GRPO full FT saved to {OUTPUT_DIR}")
