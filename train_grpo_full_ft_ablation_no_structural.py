"""
Ablation: collision + shape only (no interlocking / connectivity).
...
OUTPUT_DIR = os.getenv("OUTPUT_DIR", "finetuned_models/qwen_pc_grpo_full_ft_ablation_no_structural")
RUN_NAME = os.getenv("RUN_NAME", "brickgpt-grpo-full-ft-ablation-no-structural")
...
def main():
    ...
    if resume_checkpoint is not None:
        print(f"Resuming GRPO full FT (no-structural ablation) from {resume_checkpoint}")
        trainer.train(resume_from_checkpoint=resume_checkpoint)
    else:
        trainer.train()
    trainer.save_model(OUTPUT_DIR)
    print(f"GRPO full FT (no-structural ablation) saved to {OUTPUT_DIR}")
