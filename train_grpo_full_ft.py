...
Starts from a full-finetuned model checkpoint and updates full model weights.
"""
import os
import sys
...
OUTPUT_DIR = os.getenv("OUTPUT_DIR", "finetuned_models/qwen_pc_grpo_full_ft")
RUN_NAME = os.getenv("RUN_NAME", "brickgpt-grpo-full-ft")
...
def main():
    ...
    if resume_checkpoint is not None:
        print(f"Resuming GRPO full FT from {resume_checkpoint}")
        trainer.train(resume_from_checkpoint=resume_checkpoint)
    else:
        trainer.train()
    trainer.save_model(OUTPUT_DIR)
    print(f"GRPO full FT model saved to {OUTPUT_DIR}")
